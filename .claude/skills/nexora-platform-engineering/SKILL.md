...
1. Read `AGENTS.md` (also imported by `CLAUDE.md`).
...
4. Read [module-catalog.md](../../../.agents/skills/nexora-platform-engineering/references/module-catalog.md)
   for ownership, contracts, dependencies, tables, permissions, and the
   Core/product boundary.
5. Read [change-checklists.md](../../../.agents/skills/nexora-platform-engineering/references/change-checklists.md)
   for planning, implementation, or review.
6. Use [the ADR template](../../../docs/adr/0000-template.md) when a material
   decision changes architecture or the product boundary.

The reference documents are shared with the Codex/`.agents` copy of this skill
so both toolchains stay on one source of truth. Edit them in
`.agents/skills/nexora-platform-engineering/references/`, never fork them here.
...
For complex work, keep integration in the main thread. When the user asks for a
delegated or parallel review, dispatch independent read-heavy checks with the
Agent tool using `subagent_type: nexora-architect`, `nexora-security-reviewer`,
or `nexora-quality-reviewer`. Each is read-only, so they never conflict with
main-thread edits. Avoid delegation for trivial work and overlapping parallel
edits.
