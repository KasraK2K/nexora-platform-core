...
describe('MailDeliveryService', () => {
  ...
  it('marks a successful claimed message sent', async () => {
    ...
    await expect(fixture.delivery.deliverNow('delivery-id')).resolves.toBe(
      true,
    );
  ...
  it('expires old messages and drains due IDs sequentially', async () => {
    const fixture = createFixture({ attemptCount: 1 });
    fixture.repository.findDueIds.mockResolvedValue([
      'first-delivery',
      'second-delivery',
    ]);
    const deliverNow = jest
      .spyOn(fixture.delivery, 'deliverNow')
      .mockResolvedValue(true);

    await fixture.delivery.drainDue(7);

    expect(fixture.repository.expireDue.mock.calls).toEqual([
      [expect.any(Date), 3],
    ]);
    const [now] = fixture.repository.expireDue.mock.calls[0];
    expect(fixture.repository.findDueIds.mock.calls).toEqual([[now, 7, 3]]);
    expect(deliverNow.mock.calls).toEqual([
      ['first-delivery'],
      ['second-delivery'],
    ]);
  });
  ...
  it('renews its fenced lease while provider delivery is still pending', async () => {
    ...
    try {
      ...
      const delivery = fixture.delivery.deliverNow('delivery-id');
  ...
  function createFixture(options: {
    ...
    return {
      repository,
      outboundMail,
      delivery: new MailDeliveryService(
        repository,
        protector,
        outboundMail,
        {
