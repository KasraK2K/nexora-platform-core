...
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| Memberships / `MembershipsRepository`                      | `find`, `findActiveById`, `findActiveForUser`, `listActive` and cursor resolution, `updateRole`, `remove`, `countActiveOwners`, `transferOwnership` | Tenant A cannot read, page from, mutate, remove, count, or promote tenant B rows; all B rows remain unchanged      |
| Membership Invitations / `MembershipInvitationsRepository` | `retireActive`, `findActiveById`, `findActiveForEmail`, `revoke`, `accept`, `markDelivery`                                                          | Tenant A scope cannot find or terminally update tenant B's invitation                                              |
| Authentication / `AuthenticationSessionsRepository` and `SessionStateRepository` | `findLatestForUser` joins an active membership; `hasActiveContext` and `revokeActiveForMembership` match workspace explicitly                       | Removed tenant context cannot anchor recovery; a mismatched workspace cannot validate or revoke tenant B's session |
| Workspaces / `WorkspacesRepository`                        | Conditional `rename` uses workspace ID plus organization ID                                                                                         | Tenant A organization scope cannot rename tenant B's workspace                                                     |
| Audit / `AuditRepository`                               | Append-only input always carries the use case's trusted workspace                                                                                   | HTTP matrix asserts tenant A actions never append tenant B audit rows                                              |
